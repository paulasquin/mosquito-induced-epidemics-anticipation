...
def load_train(train_path, image_size, classes, shorter=0, num_channels=3):
    ...
    sys.stdout.flush()
    images = np.zeros((num_images, image_size, image_size, num_channels), dtype=np.int)
    labels = np.zeros((num_images, len(classes)), dtype=np.int)
...
def read_train_sets(train_path, image_size, classes, validation_size, shorter=0, num_channels=3,
    ...
    if not refresh_data:
        print("\tLoading npy data in the memory from " + str(dataset_save_dir_path))
        try:
    ...
    if refresh_data:
        ...
        sys.stdout.flush()
        try:
            create_folder(dataset_save_dir_path)
            ...
            print("\nProblem writing the dataset. Skipping.")
            print("***\n" + str(e) + "\n***")
    ...
    sys.stdout.flush()
    data_sets.train = DataSet(images[validation_size:], labels[validation_size:], img_names[validation_size:],
                              cls[validation_size:])
    data_sets.valid = DataSet(images[:validation_size], labels[:validation_size], img_names[:validation_size],
                              cls[:validation_size])
    print(" - Done")
    del images
    del labels
